...
class Formulario
{
    ...
    public function __construct()
    {
        ...
        $this->preguntas = array(
            ...
            "¿Hubo más de un 50% de humedad en los días de entrenamiento?",
    ...
    public function cargarFormulario()
    {
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            $error = isset($this->errores[$i]) ? "<span class='error'>{$this->errores[$i]}</span>" : "";

            echo "
                <p>
                    <label for='p$i'>$i. $pregunta</label><br>
                    <input type='text' id='p$i' name='p$i' value='$valor'>
                    $error
                </p>
            ";
        }
        ...
        $error11 = isset($this->errores[11]) ? "<span class='error'>{$this->errores[11]}</span>" : "";

        echo "
            <p>
                <label for='p11'>11. {$this->preguntas[10]}</label><br>
                <textarea id='p11' name='p11' rows='4' cols='60'>$valor11</textarea>
                $error11
            </p>
        ";

        $valor12 = isset($this->respuestas[12]) ? htmlspecialchars($this->respuestas[12]) : "";
        $error12 = isset($this->errores[12]) ? "<span class='error'>{$this->errores[12]}</span>" : "";

        echo "
            <p>
                <label for='p12'>12. {$this->preguntas[11]}</label><br>
                <input type='number' id='p12' name='p12' min='0' max='10' value='$valor12'>
                $error12
            </p>
        ";
        ...
        echo "
            <p>
                <label for='p13'>13. Propuestas de mejora:</label><br>
                <textarea id='p13' name='p13' rows='4' cols='60'>$valor13</textarea>
                $error13
            </p>
        ";
